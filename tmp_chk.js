...
(function(){
  try{
    ...
    for(var i=0;i<els.length;i++){
      if(/BUILDs*vd+/i.test(els[i].textContent))els[i].innerHTML=els[i].innerHTML.replace(/vd+/i,'v62');
    }
  }catch(e){}
...
(function(){
  try{
    var els=document.querySelectorAll('.chip');
    for(var i=0;i<els.length;i++){
      if(/BUILDs*vd+/i.test(els[i].textContent))els[i].innerHTML=els[i].innerHTML.replace(/vd+/i,'v64');
    }
  }catch(e){}
})();
...
/* ---------- v76 stamp + map logo ---------- */
(function v76stamp(){var VER="v76";function work(){try{var all=document.querySelectorAll("span,div,b,small,em,button,a");for(var i=0;i<all.length;i++){var e=all[i];if(e.children.length===0&&/^s*BUILDs*vd+s*$/i.test(e.textContent)){e.textContent="BUILD "+VER}}}catch(e){}try{var host=document.querySelector(".wmap")||document.querySelector("#corrMap")||document.body;var el=host.querySelectorAll("img,svg,image");for(var k=0;k<el.length;k++){var x=el[k];if(x.getBoundingClientRect().width>60){x.style.setProperty("width","auto","important");x.style.setProperty("height","14px","important");x.style.setProperty("max-width","28px","important");x.style.setProperty("max-height","14px","important");x.style.setProperty("object-fit","contain","important")}}}catch(e){}}var n=0;var t=setInterval(function(){work();if(++n>25)clearInterval(t)},600);setTimeout(work,300);document.addEventListener("click",function(){setTimeout(work,250)},true)})();
