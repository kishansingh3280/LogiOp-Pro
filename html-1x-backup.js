...
<script>document.write('<b id=alv style="position:fixed;top:0;left:0;z-index:99999;background:red;color:#fff">ALIVE</b>');document.title='ALIVE';/*v80err*/window.onerror=function(m,src,l,c){try{var d=document.createElement("div");d.style.cssText="position:fixed;z-index:99999;left:0;right:0;bottom:0;background:#b00020;color:#fff;font:12px monospace;padding:10px;white-space:pre-wrap";d.textContent="ERR: "+m+"  @line "+l+":"+c;if(document.body)document.body.appendChild(d);else setTimeout(function(){document.body&&document.body.appendChild(d)},800)}catch(e){}};
/*v81ls*/(function(){var mem={};var fake={getItem:function(k){return Object.prototype.hasOwnProperty.call(mem,k)?mem[k]:null},setItem:function(k,v){mem[k]=String(v)},removeItem:function(k){delete mem[k]},clear:function(){mem={}},key:function(i){var a=Object.keys(mem);return i<a.length?a[i]:null},get length(){return Object.keys(mem).length}};var ok=false;try{window.localStorage.setItem('__v81t','1');window.localStorage.removeItem('__v81t');ok=true}catch(e){}if(!ok){try{Object.defineProperty(window,'localStorage',{configurable:true,get:function(){return fake}})}catch(e){}}window.__v81ls=ok?'native':'memory';})();
/* ============================================================
...
function moveGlow(btn){if(!glow)return;if(!btn)return;
...
nav&&nav.addEventListener('click',e=>{const b=e.target.closest('.nav-item');if(b)go(b.dataset.view)});
...
/* ---------- Theme ---------- */
var __tb=$('#themeBtn');__tb&&__tb.addEventListener('click',()=>{
...
(function clock(){
  const d=new Date();
  var _cc=$(String.fromCharCode(35)+"clockChip");if(_cc)_cc.textContent=d.toLocaleTimeString("en-IN",{hour:"2-digit",minute:"2-digit"});
  var _ts=$(String.fromCharCode(35)+"todayStr");if(_ts)_ts.textContent=d.toLocaleDateString("en-IN",{weekday:"long",day:"numeric",month:"short"});
...
window.__BR=$('#briefReplay');/* ---------- Boot v60 cinema (module niche define hota hai — hoisted) ---------- */
setTimeout(function(){var D=document.createElement('div');D.id='v83d';D.style.cssText='position:fixed;top:0;left:0;z-index:999999;background:#003;color:#0f0;font:11px monospace;padding:6px';D.textContent='ls='+window.__v81ls+' boot='+(!!document.querySelector('#boot'))+' rbc='+(typeof runBootCinema);document.body.appendChild(D);try{runBootCinema()}catch(e){var bt=$('#boot');if(bt)bt.style.display='none';typeBrief()}},60);
window.__BR&&window.__BR.addEventListener('click',()=>{buzz(10);typeBrief()});
...
function logoURI(code){/*v77size*/var s=LOGOS[code]||LOGOS['IATA'];s=String(s).replace(/<svg\b/i,'<svg width="64" height="18" preserveAspectRatio="xMidYMid meet"');return 'data:image/svg+xml;charset=utf-8,'+encodeURIComponent(s)}
